...
def calculate_significance(before_samples, after_samples, required_p_value=0.05):

    """
    Calculate statistical significance either via t_test_paired or wilcoxon, depending on normality.

    Params:
        before_samples (numpy.arr) - mesurements before modification
        after_samples (numpy.arr) - mesurements after modification
    ...
    normal = True
    p_value_t_test = paired_t_test(before_samples, after_samples)
    p_value_wilcoxon = -1
    after_samples_normal = is_normal_distributed(after_samples, is_uncal=False)
    before_samples_normal = is_normal_distributed(before_samples, is_uncal=True)
    if not (after_samples_normal[1] and before_samples_normal[1]):
        print(f"[ERROR] not normally distributed")
        normal = False
        p_value_wilcoxon = wilcox_test(before_samples, after_samples)
        p_value_t_test = -1
    #all_statistics_t_test.append(p_value_t_test)
    #all_statistics_wilcoxon.append(p_value_wilcoxon)
    #all_p_values.append((before_samples_normal, after_samples_normal))
    return p_value_t_test, p_value_wilcoxon, before_samples_normal, after_samples_normal, normal
...
def wilcox_test(before_samples, after_samples):
    """
    ...
    """
    result = stats.wilcoxon(before_samples, after_samples)
...
if __name__ == '__main__':
    ...
    save_file = f'CPa_statistical_significance_results_for_DeepJIT.txt'
    file_path = 'CPa_SA/DeepJIT'  # Update with your actual file path
