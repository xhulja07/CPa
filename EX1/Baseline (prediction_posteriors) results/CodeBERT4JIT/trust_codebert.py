...
folder_path = "./CodeBERT_" + dataset
csv_files = glob.glob(os.path.join(folder_path, "*.csv"))  # Changed to .csv
...
for file_path in csv_files:
    df = pd.read_csv(file_path)  # Changed from pd.read_excel()

    total_rows = len(df)

    # Correct predictions
    correct_all = ((df["p"] >= 0.5).astype(int) == df["label"].astype(int))
    correct_clean = correct_all & (df["label"].astype(int) == 0)
    correct_fault = correct_all & (df["label"].astype(int) == 1)

    # Flagged predictions
    flagged_all = (df["p"] >= confidence) | (df["p"] <= 1 - confidence)
    flagged_clean = flagged_all & ((df["p"] < 0.5).astype(int))
    flagged_fault = flagged_all & ((df["p"] >= 0.5).astype(int))
...
output_file = os.path.join("CodeBERT_" + dataset + "_detailed_results_0_95.xlsx")
